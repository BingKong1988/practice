...
def mc_prediction(policy, env, num_episodes, discount_factor=1.0):
    """
    Monte Carlo prediction algorithm. Calculates the value function fro a given policy using sampling.

    Args:
        policy: A function taht maps an observation to action probabilities.
        env: OpenAI gym environment.
        num_episodes: Number of episodes to sample.
        discount_factor: Gamma discount factor.

    Returns:
        A dictionary that maps from state to values.
        The state is a tuple and the value is a float.
    """

    # Keeps track of sum and count of returns for each state to calculate an average.
    # We could use and array to save all returns but that's memory inefficient.
    ...
    for  i_episode in range(1, num_episodes + 1):
        ...
        # Find all states that we've visited in this epsisode.
